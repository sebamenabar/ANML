...
def main(args):
    ...
    my_experiment = experiment(
        args.name, args, "../results/", commit_changes=args.commit
    )
    ...
    iterator_test = torch.utils.data.DataLoader(
        dataset_test, batch_size=5, shuffle=True, num_workers=1
    )
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--model_name", help="Name of model to be saved", default="my_sweet_model.net"
    )
    argparser.add_argument(
        "--treatment", help="Neuromodulation or OML", default="Neuromodulation"
    )
    argparser.add_argument(
        "--checkpoint", help="Use a checkpoint model", action="store_true"
    )
    argparser.add_argument(
        "--saved_model", help="Saved model to load", default="my_model.net"
    )
    ...
    argparser.add_argument(
        "--tasks", type=int, help="meta batch size, namely task num", default=1
    )
    argparser.add_argument(
        "--meta_lr", type=float, help="meta-level outer learning rate", default=1e-2
    )
    argparser.add_argument(
        "--update_lr",
        type=float,
    ...
    argparser.add_argument(
        "--update_step", type=int, help="task-level inner update steps", default=20
    )
    argparser.add_argument(
        "--name", help="Name of experiment", default="mrcl_classification"
    )
    ...
    argparser.add_argument("--prefetch_gpu", action="store_true")
